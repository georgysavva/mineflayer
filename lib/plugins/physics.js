...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.look = async (yaw, pitch, force, yawSpeedOverrideParam, pitchSpeedOverrideParam) => {
    if (!lookingTask.done) {
      lookingTask.finish() // finish the previous one
    }
    lookingTask = createTask()

    // Set speed overrides if provided
    yawSpeedOverride = yawSpeedOverrideParam ?? null
    pitchSpeedOverride = pitchSpeedOverrideParam ?? null
    ...
    if (yawChange === 0 && pitchChange === 0) {
      // Clear overrides if no movement needed
      yawSpeedOverride = null
      pitchSpeedOverride = null
      return
    }
  ...
  bot.lookAt = async (point, force, yawSpeedOverride, pitchSpeedOverride) => {
    const delta = point.minus(bot.entity.position.offset(0, bot.entity.eyeHeight, 0))
    const yaw = Math.atan2(-delta.x, -delta.z)
    const groundDistance = Math.sqrt(delta.x * delta.x + delta.z * delta.z)
    const pitch = Math.atan2(delta.y, groundDistance)
    await bot.look(yaw, pitch, force, yawSpeedOverride, pitchSpeedOverride)
  }
