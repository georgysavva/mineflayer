...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.getLastCameraAction = () => {
    if (lastRecordedYawPitch[0] === null || lastRecordedYawPitch[1] === null) {
      ...
      return {
        delta: [0.0, 0.0],
        absolute: [lastSentYaw, lastSentPitch]
      }
    } else {
      const diff = [deltaYaw(lastSentYaw, lastRecordedYawPitch[0]), lastSentPitch - lastRecordedYawPitch[1]]
      lastRecordedYawPitch = [lastSentYaw, lastSentPitch]
      return {
        delta: diff,
        absolute: [lastSentYaw, lastSentPitch]
  ...
  function updatePosition (now) {
    ...
    // Wrap yaw to [0, 2*PI) radians by round-tripping through Notchian degrees
    lastSentYaw = conv.fromNotchianYaw(conv.toNotchianYaw(lastSentYaw))
  ...
  bot.look = async (yaw, pitch, force, yawSpeedOverrideDPS, pitchSpeedOverrideDPS, useEasing = false) => {
    ...
    // Wrap yaw to [0, 2*PI) radians to match fromNotchianYaw convention
    bot.entity.yaw = conv.fromNotchianYaw(conv.toNotchianYaw(bot.entity.yaw))
    bot.entity.pitch += pitchChange

    if (force) {
      // Wrap yaw to [0, 2*PI) radians to match fromNotchianYaw convention
      lastSentYaw = conv.fromNotchianYaw(conv.toNotchianYaw(yaw))
      lastSentPitch = pitch
  ...
  bot.waitForTicks = async function (ticks) {
    ...
    await new Promise(resolve => {
      let timeout
      // Common cleanup function to remove listener, clear timer, and resolve
      const finish = () => {
      ...
      const tickListener = () => {
        ...
        if (ticks === 0) {
          ...
          // Reset the watchdog timer for the next tick
