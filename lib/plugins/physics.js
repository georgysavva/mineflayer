...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  let lookEasing = null // { startYaw, startPitch, targetYaw, targetPitch, startTime, duration }
  ...
  // Speed ranges from 0.4 to 1.0 (40% to 100% of max speed)
  // Average speed multiplier: 0.88
  ...
  function trapezoidEasing (t) {
    if (t < 0.2) {
      // Ramp up: v(t) = 0.4 + 3t
      // Position integral: 0.4t + 1.5t²
      return (0.4 * t + 1.5 * t * t) / 0.88
    } else if (t < 0.8) {
      // Constant speed: v(t) = 1.0
      // Position integral: 0.14 + (t - 0.2)
      return (t - 0.06) / 0.88
    } else {
      // Ramp down: v(t) = 3.4 - 3t
      // Position integral: -1.02 + 3.4t - 1.5t²
      return (-1.02 + 3.4 * t - 1.5 * t * t) / 0.88
    }
  }
  ...
  bot.getLastCameraAction = () => {
    ...
    // If using easing, calculate delta based on eased position
    if (lookEasing) {
      const elapsed = performance.now() - lookEasing.startTime
      const t = Math.min(elapsed / lookEasing.duration, 1.0)
      ...
      dPitch = (lookEasing.startPitch + totalPitchDelta * easedProgress) - lastSentPitch
    } else {
      // Original logic: delta to final target
    ...
    return [
      math.clamp(-maxDeltaYaw, dYaw, maxDeltaYaw),
      math.clamp(-maxDeltaPitch, dPitch, maxDeltaPitch),
    ];
  };
  ...
  function updatePosition (now) {
    ...
    if (lookEasing) {
      const elapsed = now - lookEasing.startTime
      const t = Math.min(elapsed / lookEasing.duration, 1.0)
      const easedProgress = trapezoidEasing(t)
      ...
      // the dYaw for this tick is the expected yaw minus the last sent yaw
      dYaw = deltaYaw(lookEasing.startYaw + totalYawDelta * easedProgress, lastSentYaw)
  ...
  bot.look = async (yaw, pitch, force, yawSpeedOverrideDPS, pitchSpeedOverrideDPS, useEasing = false) => {
    ...
    if (useEasing) {
      ...
      // Calculate duration based on trapezoid average speed (0.88)
      const yawDuration = Math.abs(yawChange) / (yawSpeed * 0.88) * 1000
      const pitchDuration = Math.abs(pitchChange) / (pitchSpeed * 0.88) * 1000
      const duration = Math.max(yawDuration, pitchDuration)
  ...
  bot.lookAt = async (point, force, yawSpeedOverride, pitchSpeedOverride, useEasing = false) => {
    const delta = point.minus(bot.entity.position.offset(0, bot.entity.eyeHeight, 0))
    const yaw = Math.atan2(-delta.x, -delta.z)
    const groundDistance = Math.sqrt(delta.x * delta.x + delta.z * delta.z)
    const pitch = Math.atan2(delta.y, groundDistance)
    await bot.look(yaw, pitch, force, yawSpeedOverride, pitchSpeedOverride, useEasing)
  }
