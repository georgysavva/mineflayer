...
function inject (bot) {
  ...
  async function dig (block, forceLook, digFace) {
    ...
    if (block.name.includes('ore') && block.material === 'incorrect_for_wooden_tool') {
      // copy block and set material to mineable/pickaxe to patch wrong material
      old_material = block.material
      block.material = 'mineable/pickaxe'
    }
    const waitTime = bot.digTime(block)
    block.material = old_material
    ...
    let diggingContinues = false
    if (bot.targetDigBlock) {
      const stoppedBecauseOfNewDigRequest = bot.stopDigging()
      diggingContinues = stoppedBecauseOfNewDigRequest
    }

    if (!diggingContinues) {
      bot.emit('action_mine_start')
    }
    ...
    bot.stopDigging = () => {
      ...
      if (!stoppedBecauseOfNewDigRequest) {
        bot.emit('action_mine_finish')
      }
      ...
      return stoppedBecauseOfNewDigRequest
    }
